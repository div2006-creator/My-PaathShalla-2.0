...
function StudentDashboard({ user }) {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 space-y-6">
      ...
      <section className="bg-gradient-to-r from-indigo-900 via-indigo-800 to-slate-900 text-white p-6 sm:p-8 rounded-2xl shadow-xl border border-indigo-500/20 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="space-y-2 max-w-2xl">
          ...
          <h2 className="text-2xl sm:text-3xl font-extrabold font-display leading-tight text-white">
            {liveClass ? liveClass.subject : 'PaathShalla Digital Classroom'}
          </h2>
            ...
            {schedule.map((item) => (
              <div key={item.id} className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-sm space-y-3">
                ...
                <div className="pt-3 border-t border-slate-800 flex justify-between items-center">
                  ...
                  <Link
                    href={`/live?subject=${encodeURIComponent(item.subject)}&topic=${encodeURIComponent(item.topic)}`}
      ...
      {/* 3. PENDING WORK & RECENT RECORDINGS */}
        ...
        <section className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-sm space-y-4">
          ...
          {pendingAssignments.length > 0 ? (
            <div className="space-y-3">
              {pendingAssignments.map((a) => (
                <div key={a.id} className="p-3.5 bg-slate-800/50 border border-slate-800 rounded-xl flex justify-between items-center gap-4">
                  <div>
                    <span className="text-[10px] font-bold uppercase text-amber-400">{a.subject}</span>
                    <h4 className="font-bold text-white text-xs truncate max-w-xs">{a.title}</h4>
                    <p className="text-[10px] text-slate-400">Due: {new Date(a.dueDate).toLocaleDateString()}</p>
                  </div>
                  <Link href="/assignments" className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl shadow-sm shrink-0">
                    VIEW
                  </Link>
                </div>
              ))}
            ...
            <div className="p-6 text-center text-slate-400 space-y-1">
              ...
              <p className="font-bold text-xs text-white">No assignments available</p>
              <p className="text-[11px]">You're all caught up with your coursework!</p>
            </div>
          )}
        </section>
        ...
        <section className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-sm space-y-4">
          <div className="flex justify-between items-center border-b border-slate-800 pb-3">
            <h3 className="text-base font-bold text-white font-display flex items-center gap-2">
              ...
              <span>Recent Class Recordings</span>
            </h3>
          ...
          {recordings.length > 0 ? (
            <div className="space-y-3">
              {recordings.slice(0, 3).map((r) => (
                <div key={r.id} className="p-3.5 bg-slate-800/50 border border-slate-800 rounded-xl flex justify-between items-center gap-4">
                  <div>
                    <span className="text-[10px] font-bold uppercase text-amber-400">{r.subject}</span>
                    <h4 className="font-bold text-white text-xs truncate max-w-xs">{r.title}</h4>
                    <p className="text-[10px] text-slate-400">Duration: {r.duration}</p>
                  </div>
                  <Link href="/recordings" className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl shadow-sm shrink-0">
                    WATCH
                  </Link>
                </div>
              ))}
            ...
            <div className="p-6 text-center text-slate-400 space-y-1">
              ...
              <p className="text-[11px]">Recordings will appear here after your live classes.</p>
            </div>
          )}
        </section>
...
function TeacherDashboard({ user }) {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 space-y-6">
      ...
      {/* 2. DYNAMIC OVERVIEW STATISTICS CARDS */}
      <section className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl space-y-1">
          <span className="text-[10px] font-bold text-slate-400 uppercase">Scheduled Classes</span>
          <p className="text-2xl font-black text-indigo-400">{schedule.length}</p>
          <span className="text-[10px] text-slate-400 font-bold">Active Timetable</span>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl space-y-1">
          ...
          <span className="text-[10px] text-slate-400 font-bold">Published Coursework</span>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl space-y-1 col-span-2 md:col-span-1">
          ...
          <span className="text-[10px] text-emerald-400 font-bold">Auto Tracking</span>
        </div>
      </section>
      ...
      <section className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-sm space-y-4">
        ...
        {schedule.length > 0 ? (
          <div className="space-y-3">
            {schedule.map((item) => (
              <div key={item.id} className="p-4 bg-slate-800/40 border border-slate-800 rounded-xl flex justify-between items-center gap-4">
                ...
                <Link
                  href={`/live?room=${item.id}&subject=${encodeURIComponent(item.subject)}`}
          ...
          <div className="p-8 text-center text-slate-400 space-y-2">
            ...
            <Link href="/teacher/classes" className="inline-block mt-2 px-4 py-2 bg-amber-500 text-black font-extrabold text-xs rounded-xl">
              Create New Class
            </Link>
          </div>
        )}
